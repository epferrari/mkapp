...
function promptConfigOverwrite(){
	return new Promise(function(resolve,reject){
		if(shell.test('-f','./mkapp_config.js')){
			process.stdout.write('\x07');
			console.log(clc.yellow('A mkapp_config.js file already exists in your project. Overwrite it?'));
			yesOrNo('Overwrite mkapp_config.js?'.yellow,'N')
				.then(resolve)
				.catch(function(err){
					return reject('SKIP');
				});
...
module.exports = function downloadBoilerplate(installedMkappVersion){
	...
	return promptBoilerplateDownload()
		...
		.then(function(){
			var reason;
			return new Promise(function(resolve,reject){
				if(!shell.test('-f',tmpdir+'/mkapp-master/mkapp_config.js')){
					reason = reportIssue('No config file found in downloaded project.')
					reject(reason);
					return;
				}
				var copied = shell.cp('-f',tmpdir+'/mkapp-master/mkapp_config.js','./');
				if(copied && copied.code !== 0){
					reason = reportIssue('could not copy mkapp_config.js to project root.');
					reject(reason);
				}else{
					resolve();
				}
		...
		.then(function(){
			if(!shell.test('-f','./mkapp_config.js')){
				var msg = "Config file not present! You MUST create a mkapp_config.js file in your project root. See documentation for example project structure.";
				console.log(clc.red(msg));
				console.log('...continuing. Don\'t forget to create this file before developing');
			}else{
				console.log('...continuing with present mkapp_config.js');
			}
		})
		.finally(function(){
